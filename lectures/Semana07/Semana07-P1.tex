...
\begin{frame}{Repaso de la Sesión 12}
  \begin{itemize}
    \item \textbf{Semana 6, Sesión 2 (Sesión 12)} se enfocó en:
      \begin{itemize}
        \item Manipulación avanzada de NumPy (\texttt{reshape}, \texttt{linspace}, \texttt{arange}).
        \item Uso de \texttt{np.random} para valores aleatorios y \texttt{np.std} (álgebra lineal).
      \end{itemize}
    \item \textbf{Objetivo de hoy}: Avanzar con gráficos (plots, scatter, histogramas) y comenzar a manipular datos de forma más eficiente (posiblemente una introducción a \texttt{pandas}).
  \end{itemize}
\end{frame}
...
\begin{minted}[fontsize=\tiny,baselinestretch=0.85]{python}
import matplotlib.pyplot as plt
import numpy as np
